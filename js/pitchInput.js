...
{	
    ...
    */
    $('.pitch_input').on('change', '[id^=areaPitch]', function () {
        var UpperNoteLimit = 2000; //  This is the max note count.
	    var $panel = $(this).closest('div[id]');
	    var $NoteCount = $panel.find('[id^=note_count]');
	    var $TextBox = $panel.find('[id^=areaPitch]');
	    var voiceNumber = getVoiceNumber($panel); // getVoiceNumber is in WebSiteFunctions.js
	    ...
	    if(candidateArray.length > UpperNoteLimit)
	    {
            ...
            // If the length decreases, we use weird javascript technique of reducing the array length.
	...
	*/
	// this may be duplicated in WebSiteFunctions.js
